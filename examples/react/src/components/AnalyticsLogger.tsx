...
export default function AnalyticsLogger() {
  ...
  return (
    <section>
      <h2>Analytics Log <span style={{ fontSize: 12, fontWeight: 'normal', color: '#888' }}>(useAnyEvent — catches all events)</span></h2>
        ...
        {logs.length === 0 ? (
          ...
          logs.map((log, i) => (
            <div key={i} style={{ padding: '2px 0' }}>
              ...
              {log.payload != null && <span style={{ color: '#ce9178' }}> {JSON.stringify(log.payload)}</span>}
            </div>
          ))
        )}
      </div>
