...
51. ✅ Middleware package scaffolding — `packages/middleware/` with `package.json` (`@tiny-event-bus/middleware`), peer dep on `@tiny-event-bus/core`, dual ESM/CJS exports, `tsconfig.json`, `tsconfig.cjs.json`, `vitest.config.ts` (90% thresholds), `src/types.ts` (`Middleware<T>`, `MiddlewareNext<T>`, `MiddlewareBus<T>`), `src/with-middleware.ts` (shell decorator delegating all 8 IEventBus methods), `src/index.ts`, `README.md`, skeleton delegation tests, update AGENTS.md + README.md + PLUGIN_ARCHITECTURE.md
52. ⬜ Basic emit interception — TDD: middleware is called on `emit()`, calling `next()` propagates to handlers, not calling `next()` blocks the event, middleware can mutate data passed to `next()`, middleware can change event name passed to `next()`, update AGENTS.md
...
54. ⬜ Dynamic `use()` — TDD: `use(middleware)` adds middleware at runtime, returned `Unsubscribe` removes it, double-unsubscribe is a no-op, initial-array middlewares run before dynamically-added ones, multiple dynamic middlewares added/removed independently, update AGENTS.md
55. ⬜ Composition + edge cases — TDD: `withMiddleware(withReplay(bus))` blocked events not in replay history, `withReplay(withMiddleware(bus))` buffers before middleware, re-entrant emit safety, `use` is own enumerable property, update AGENTS.md + PLUGIN_ARCHITECTURE.md + middleware README
