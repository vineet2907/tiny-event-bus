...
43. ⬜ Generic `useEventBus` — refactor `createEventBus` to return object literal typed as `IEventBus<T>` (remove `EventBus` class export), migrate core tests from `new EventBus()` to `createEventBus()`, fix `BusInspector.tsx` type, then refactor `use-event-bus.ts` to dynamically discover and wrap all bus methods via `Object.keys` + `BusMethods<B>` mapped type, backward compat with plain bus, extended bus exposes plugin methods, stable refs via `useMemo`, update AGENTS.md + READMEs
